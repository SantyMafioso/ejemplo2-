...
const updateProductos = async (req, res) => {
    const { id, nombre, description, precio, stock } = req.body;try {
        ...
        if (req.files.image) {
        await deleteImage(updateProducto.image.public_id);const result = await uploadImage(req.files.image.tempFilePath);await fs.remove(req.files.image.tempFilePath);
